...
const LayerPreview: React.FC<LayerPreviewProps> = memo(({ imageData, width, height, label, tint, onPixelSelect, className, fitContain, forceBackground, viewMode = 'color' }) => {
  ...
  useEffect(() => {
    ...
    if (viewMode === 'mask') {
      // CPU Alpha Mask implementation using Uint32Array for performance
      ...
      for (let i = 0; i < src32.length; i++) {
        // Extract Alpha (top 8 bits in Little Endian is alpha)
        const a = (src32[i] >> 24) & 0xFF;
        // Output format is Little Endian: A B G R
        // We want R=a, G=a, B=a, A=255
        dst32[i] = (255 << 24) | (a << 16) | (a << 8) | a;
      }
      ctx.putImageData(newImg, 0, 0);

    } else if (viewMode === 'positive') {
      // CPU Film Positive implementation using Uint32Array
      const newImg = ctx.createImageData(width, height);
      const src32 = new Uint32Array(imageData.data.buffer);
      const dst32 = new Uint32Array(newImg.data.buffer);
      ...
      for (let i = 0; i < src32.length; i++) {
        ...
        const val = 255 - a;
        // Output format is Little Endian: A B G R
        // We want R=val, G=val, B=val, A=255
        dst32[i] = (255 << 24) | (val << 16) | (val << 8) | val;
      }
      ctx.putImageData(newImg, 0, 0);

    } else if (tint) {
      // CPU Tinting implementation using Uint32Array
      // Parse Hex Tint into RGB
      const rT = parseInt(tint.slice(1, 3), 16);
      const gT = parseInt(tint.slice(3, 5), 16);
      const bT = parseInt(tint.slice(5, 7), 16);

      // Construct color mask A B G R. Alpha defaults to 0, we apply it in the loop.
      const colorMask = (bT << 16) | (gT << 8) | rT;
