...
import { X, ChevronLeft, ChevronRight, Scissors, Link, Edit2, Trash2, Palette, ScanFace, FileText, Sparkles } from 'lucide-react';
...
interface LayerDetailModalProps {
    ...
    onAction: (action: 'chop' | 'merge' | 'edit' | 'delete' | 'refine', layer: Layer) => void;
    isHalftone: boolean;
}
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-4 md:p-8 animate-in fade-in duration-200" onClick={onClose}>
            ...
            <div className="flex flex-col items-center gap-6 max-w-5xl w-full" onClick={(e) => e.stopPropagation()}>
                ...
                {/* Action Toolbar */}
                <div className="grid grid-cols-5 gap-3 w-full max-w-3xl mt-2">
                    ...
                    <div className="flex flex-col gap-1">
                        <Button
                            variant="primary"
                            onClick={() => onAction('refine', layer)}
                            className="flex flex-col py-4 h-auto gap-2 bg-amber-600 hover:bg-amber-700 border-b-4 border-amber-800 hover:border-amber-900"
                        >
                            <Sparkles className="w-6 h-6" />
                            <span className="text-xs font-bold uppercase tracking-wider">Refine</span>
                        </Button>
                        <span className="text-[10px] text-gray-500 text-center">Clean Noise</span>
                    </div>
                </div>
