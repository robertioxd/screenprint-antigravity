/**
 * ZIP Export Service
 * Bundles all color separation layers as individual VECTOR EPS files
 * into a single downloadable ZIP archive.
 */

import JSZip from 'jszip';
import type { Layer, AdvancedConfig } from '../types';
import { layerToEPS, generateEPSFilename, layersToCompositeEPS } from './epsExport';
...
export async function exportLayersAsEPS(
    ...
    compositeData: ImageData | null = null
): Promise<void> {
    const zip = new JSZip();
    ...
    for (let i = 0; i < visibleLayers.length; i++) {
        ...
        try {
            const epsBlob = await layerToEPS(layer, config);
            const filename = generateEPSFilename(i + 1, layer.color.hex);
            zip.file(filename, epsBlob);
        } catch (err) {
            console.error(`Failed to export layer ${i + 1}`, err);
        }
    }

    // Export composite by tracing all active layers and combining them into a single colored EPS
    try {
        const compositeBlob = await layersToCompositeEPS(visibleLayers, config);
        zip.file('00_COMPOSITE.eps', compositeBlob);
    } catch (err) {
        console.error("Failed to export composite", err);
    }
...
function generatePrintInfo(layers: Layer[], config: AdvancedConfig): string {
    ...
    lines.push('');
    lines.push('--- Notes ---');
    lines.push('Each EPS file is a TRUE VECTOR file (Bezier curves, not raster):');
    lines.push('  Black fills = Ink present (vector paths)');
    lines.push('  Fully scalable to any size without pixelation');
    lines.push('');
    lines.push('Compatible with: Adobe Illustrator, CorelDRAW, I-Image CTS, AccuRIP, FilmMaker');
    lines.push('');
